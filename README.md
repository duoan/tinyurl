...
graph TB
    subgraph Kubernetes Cluster
        ...
        subgraph "Database Layer"
            ...
            PG_REPLICA1((PostgreSQL Replica))
            PG_REPLICA2((PostgreSQL Replica))
            PG_CONFIG{PostgreSQL ConfigMap}
            PG_PVC_PRIMARY[(Primary PVC)]
            PG_PVC_REPLICA[(Replica PVC)]
        end
        ...
        subgraph "Monitoring"
            PROM((Prometheus Pod))
            PROM_CONFIG{Prometheus ConfigMap}
        end

        SVC_APP[TinyURL Service]
        SVC_PROM[Prometheus Service]
        SVC_PG_PRIMARY[PG Primary Service]
        SVC_PG_REPLICA[PG Replica Service]
        SVC_REDIS[Redis Service]

        %% Application connections
        SVC_APP --> APP1 & APP2 & APP3

        %% Database connections
        SVC_PG_PRIMARY --> PG_PRIMARY
        SVC_PG_REPLICA --> PG_REPLICA1 & PG_REPLICA2
        PG_CONFIG -.-> PG_PRIMARY & PG_REPLICA1 & PG_REPLICA2
        PG_PVC_PRIMARY -.-> PG_PRIMARY
        PG_PVC_REPLICA -.-> PG_REPLICA1 & PG_REPLICA2
        ...
        %% App to DB and Cache
        APP1 & APP2 & APP3 --> SVC_PG_PRIMARY
        APP1 & APP2 & APP3 --> SVC_PG_REPLICA
        APP1 & APP2 & APP3 --> SVC_REDIS
    ...
    class INGRESS,APP1,APP2,APP3,PG_PRIMARY,PG_REPLICA1,PG_REPLICA2,REDIS,PROM k8s_pod;
    class SVC_APP,SVC_PROM,SVC_PG_PRIMARY,SVC_PG_REPLICA,SVC_REDIS k8s_svc;
    class PG_CONFIG,PROM_CONFIG k8s_cm;
    class PG_PVC_PRIMARY,PG_PVC_REPLICA k8s_pvc;

    classDef k8s_pod fill:#326CE5,stroke:#fff,stroke-width:2px,color:#fff;
    classDef k8s_svc fill:#FF6B6B,stroke:#fff,stroke-width:2px,color:#fff;
    classDef k8s_cm fill:#FFA630,stroke:#fff,stroke-width:2px,color:#fff;
    classDef k8s_pvc fill:#4CAF50,stroke:#fff,stroke-width:2px,color:#fff;
